      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-background md:hidden flex flex-col justify-center px-6"
          >
            <nav>
              <ul className="flex flex-col gap-6">
                {links.map((link) => (
                  <motion.li key={link.href} variants={linkVariants}>
                    <Link
                      href={link.href}
                      className="text-5xl font-bold tracking-tighter text-foreground hover:text-accents-5 transition-colors block"
                      onClick={() => setIsOpen(false)}
                    >
                      {link.name}
                    </Link>
            ...
            <motion.div
              ...
              className="mt-12 pt-12 border-t border-accents-2 text-accents-5"
            >
              <p className="text-sm uppercase tracking-widest mb-4">
                {t.header.getInTouch}
              </p>
              <a
                href={`mailto:${CONTACT_INFO.emailAlt}`}
